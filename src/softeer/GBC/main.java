...
public class main {
    ...
    static class GbcData {
        int length;
        int speed;

        public GbcData(int length, int speed) {
            this.length = length;
            this.speed = speed;
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<GbcData> limitSpeedList = new ArrayList<>();
        for (int i = 0 ; i < N ; i ++) {
            st = new StringTokenizer(br.readLine());
            GbcData data = new GbcData(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()));
            limitSpeedList.add(data);
        }

        List<GbcData> realSpeedList = new ArrayList<>();
        for (int i = 0 ; i < M ; i ++) {
            st = new StringTokenizer(br.readLine());
            GbcData data = new GbcData(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()));
            realSpeedList.add(data);
        }
        int[] limitResult = convertToResult(limitSpeedList);
        int[] realResult = convertToResult(realSpeedList);

        int maxOverSpeed = Integer.MIN_VALUE;
        for (int i = 0; i < 100; i++) {
            maxOverSpeed = Math.max(realResult[i] - limitResult[i] < 0 ? 0 : realResult[i] - limitResult[i], maxOverSpeed);
        }
    ...
    private static int[] convertToResult(List<GbcData> limitSpeedList) {
        int[] result = new int[100];
        int startIndex = 0;
        for (GbcData gbcData : limitSpeedList) {
            int endIndex = startIndex + gbcData.length;
            for (int i = startIndex; i < endIndex; i ++) {
                result[i] = gbcData.speed;
            }
            startIndex += gbcData.length;
        }
        return result;
    }
}
